...
interface Category { _id: string; name: string; slug: string; description?: string; icon?: string; }
...
export default function AdminCategoriesPage() {
  ...
  const fetchItems = () => {
    setLoading(true);
    fetch("/api/categories").then(r => r.json()).then(d => { setItems(d.categories || []); setLoading(false); }).catch(() => setLoading(false));
  };
  useEffect(() => { fetchItems(); }, []);

  const openNew = () => { setEditId(null); setForm({ name: "", slug: "", description: "", icon: "" }); setShowForm(true); };
  const openEdit = (c: Category) => { setEditId(c._id); setForm({ name: c.name, slug: c.slug, description: c.description || "", icon: c.icon || "" }); setShowForm(true); };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-2xl text-white font-bold">Categories</h1>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-luxury-card border border-orthodox-gold/20 rounded-lg p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-serif text-lg text-white font-semibold">{editId ? "Edit" : "New"} Category</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className="block text-xs text-white/50 font-sans uppercase tracking-wider mb-1">Name *</label>
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required placeholder="Icons" className={inputClass} /></div>
              ...
              <div><label className="block text-xs text-white/50 font-sans uppercase tracking-wider mb-1">Icon Name</label>
                <input value={form.icon} onChange={e => setForm(f => ({ ...f, icon: e.target.value }))} placeholder="Paintbrush" className={inputClass} /></div>
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="bg-luxury-card h-14 rounded-lg animate-pulse" />)}</div>
      ) : items.length === 0 ? (
        <div className="text-center py-16 parchment-card-dark rounded-lg"><p className="text-white/30 font-serif text-lg">No categories yet</p></div>
      ) : (
        <div className="bg-luxury-card rounded-lg border border-orthodox-gold/10 overflow-hidden">
          <table className="w-full text-sm font-sans">
            ...
            <tbody>{items.map(c => (
              <tr key={c._id} className="border-b border-orthodox-gold/5 hover:bg-white/[0.02] transition-colors">
